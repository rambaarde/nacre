 ...
 * node: builtins only.
 *
 * The frontmatter reader is deliberately minimal. repertory's store is markdown
 ...
 *
 * repertory was never Claude-only by design; this table simply had two rows.
 */
...
/**
 * Walk up from `start` looking for `.repertory.yml`.
 *
...
export async function initGit(dir: string, remote?: string | null): Promise<string | null> {
  try {
    ...
    // "does not look like a repertory memory". Silent, and only ever hit by the
    // second person.
    //
 ...
 *
 * `repertory add` used to end with "teammates then need nothing", which is true of
 * `.repertory.yml` and false of everything else until the memory itself is pushed.
...
> {
    ...
    if (networked && /permission denied|access rights|authentication failed|not found|could not read from remote|repository not found/i.test(raw)) {
      return {
        ...
        reason:
          ...
          "Ask whoever set it up to run: repertory invite <your-github-username>",
      };
    }
  ...
  return {
    ...
    reason: bare
      ...
      : `${remote} has no _company.md at the top level, so it is not a repertory memory · ` +
        "check the memory URL in .repertory.yml",
  };
}
 ...
 * mean, and it was the one thing resolution never checked: `repertory serve` run
...
export async function resolveStoreDir(flag?: string, url?: string | null): Promise<string> {
  ...
  if (process.env.REPERTORY_STORE) return resolve(process.env.REPERTORY_STORE);
  ...
  // answer: `.repertory.yml` is the record of where the memory lives, and a path
...
export async function addToRoster(store: string, project: string, repos: string[]) {
  ...
  if (!roster) throw new Error(`no project "${project}" in ${store} · run: repertory add ${project}`);
...
export const SKILLS = ["rtr-l", "rtr-p"] as const;
...
function asRule(skill: string, text: string): string {
  const fm = frontmatter(text);
  const description = String(fm.description ?? `repertory — ${skill}`)
    .replace(/\s+/g, " ")
    .trim();
