 ...
 *
 * Why it is read-only: `rtr-p` is safe because a person sees the file
    ...
    inputSchema: {
      ...
      properties: {
        project: {
          type: "string",
          description:
            "Project name. Omit to use the .repertory.yml found by walking up from the " +
            "working directory.",
        },
      },
...
async function resolve(project: string | undefined, memoryFlag?: string): Promise<{ memory: string; project: string } | string> {
  ...
  if (!chosen) {
    return (
      "No project given, and no .repertory.yml found by walking up from " +
      `${process.cwd()}.\nPass project, or run repertory add <project> <repo-dir> in this repo.`
    );
  }
  try {
    const memory = await resolveStoreDir(memoryFlag, binding?.store ?? null);
    if (!(await exists(memory))) {
      return `The memory is not cloned here yet (looked in ${memory}).\nnext: repertory init <git-url>, or clone it beside your repos.`;
    }
  ...
  switch (method) {
    case "initialize": {
      ...
      return ok(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "repertory", version: ctx.version ?? "0.0.0" },
        instructions:
          "Call repertory_brief at the start of a session, before writing code. It is " +
          ...
          "rtr-p skill or the repertory CLI.",
      });
    }
    case "ping":
      return ok(id, {});
    ...
    case "tools/call": {
      ...
      try {
        if (name === "repertory_brief") return toolText(id, await brief(scope.memory, scope.project));
        const query = String(args.query ?? "").trim();
        if (!query) return toolText(id, "repertory_search needs a query.", true);
        return toolText(
          id,
