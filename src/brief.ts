 ...
 *
 * Lives apart from any one door because three of them now read it: `repertory brief`
 ...
 *
 * The order is the one rtr-l uses: urgency of not knowing, not recency.
 */
...
function cap(text: string, limit: number, note = ""): string {
  ...
  const notice = (missing: number) =>
    ...
    `${note}\`repertory search <term>\` reads the rest.]`;
 ...
 *
 * A dropped constraint is invisible, and `repertory search` is no remedy for it: a
...
function fitEntries(entries: Entry[], limit: number): string {
  ...
  const shrunkNote = (n: number) =>
    ...
    "`repertory search <term>` reads one in full.]";
  ...
  const droppedNote = (n: number, span: string) =>
    ...
    "`repertory search <term>`, and consider superseding what no longer holds.]";
...
/**
 * The briefing, in the order `rtr-l` reads it: urgency of not knowing,
 * not recency.
 */
  ...
  if (view.supersededEntries) {
    parts.push(
      `[${view.supersededEntries} constraint${view.supersededEntries === 1 ? "" : "s"} in superseded logs ` +
        ...
        "had to be restated in the log that replaced it. `repertory search <term>` reads the retired ones.]",
    );
  }
