...
export async function notify(
  text: string,
  { url = process.env.REPERTORY_NOTIFY_URL, timeoutMs = 5_000 }: { url?: string; timeoutMs?: number } = {},
): Promise<NotifyResult> {
  if (!url) return { sent: false, unset: true, reason: "REPERTORY_NOTIFY_URL is not set" };
