...
name: portal-walk
description: >
  Review repertory's portal the way a developer actually uses it — in a real
...
If there is no such memory to hand, build one: `repertory init`, `repertory add` two
...
```bash
npm run build
node dist/bin/repertory.js serve --memory <path-to-a-memory> &
```
