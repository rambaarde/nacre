...
async function withHome<T>(body: (home: string) => Promise<T>): Promise<T> {
  const home = await realpath(await mkdtemp(join(tmpdir(), "repertory-test-")));
  ...
  const realStore = process.env.REPERTORY_STORE;
  const cwd = process.cwd();
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  delete process.env.REPERTORY_STORE;
  // Also move into the temp home. Binding resolution walks up from the working
  // directory by design, so a .repertory.yml anywhere above the repo would leak in
  // — and the repo of anyone who actually uses repertory has one.
  process.chdir(home);
  try {
    ...
    if (realStore) process.env.REPERTORY_STORE = realStore;
...
test("resolveBinding walks up, and returns null rather than guessing", async () => {
  await withHome(async (home: string) => {
    ...
    await writeFile(join(home, "repo", ".repertory.yml"), "project: atlas\nstore: git@h:a/s.git\n");
...
test("init → add wires both records", async () => {
  await withHome(async (home: string) => {
    ...
    const binding = await readFile(join(repo, ".repertory.yml"), "utf8");
...
test("rebinding a repo to a different project is refused", async () => {
  await withHome(async (home: string) => {
    ...
    assert.match(await readFile(join(repo, ".repertory.yml"), "utf8"), /project: atlas/);
  });
});
...
test("addToRoster on a missing project says how to fix it", async () => {
  await withHome(async () => {
    const store = await initStore({ store: "git@github.com:acme/acme-context.git", who: "alice" });
    await assert.rejects(() => addToRoster(store.dir, "nope", ["r"]), /repertory add nope/);
  });
});
...
test("a bound repo fetches its memory instead of asking for init", async () => {
  ...
  await withHome(async (home: string) => {
    ...
    await mkdir(repo, { recursive: true });
    await writeFile(join(repo, ".repertory.yml"), `project: acme\nmemory: ${origin}\n`);
...
test("the author slug is one value, so a person cannot become two", async () => {
  ...
  // profile, and nothing errors. `repertory` therefore reports the slug it uses, so
...
test("--version and --help work from the PACKAGED layout, not just the repo", async () => {
  // `repertory --version` shipped broken: bin/repertory.ts computed its own PKG_ROOT by
  ...
  const { promisify } = await import("node:util");
  const run = promisify(execFile);
  const bin = join(import.meta.dirname, "..", "bin", "repertory.js");
  ...
  assert.match(help, /repertory init <git-url>/, "--help prints usage");
});
...
test("standing inside a memory is enough to find it", async () => {
  // `repertory serve` run from the root of a memory reported "no memory here, and
...
test("every known agent installs, in its own format", async () => {
  const home = await realpath(await mkdtemp(join(tmpdir(), "repertory-agents-")));
...
test("linking a repo leaves a note any agent can find", async () => {
  ...
  await withHome(async (home: string) => {
    ...
    assert.match(note, /repertory brief/);
    assert.match(note, /atlas/);
  });
});

test("the note is replaced, never appended twice", async () => {
  // `repertory add` is meant to be re-run — adding a repo later is the same command
  ...
  await withHome(async (home: string) => {
    ...
    const note = await readFile(join(repo, "AGENTS.md"), "utf8");
    assert.equal(note.match(/repertory:start/g)?.length, 1, note);
...
test("the publish skill still documents the retry", async () => {
  ...
  const skill = await readFile(join(PKG_ROOT_FOR_TEST, "skills/rtr-p/SKILL.md"), "utf8");
...
test("a GitHub memory yields a slug; anything else is left alone", async () => {
  ...
  // would fail in a way that reads as repertory being broken.
...
test("brief fetches the memory rather than telling a teammate to init", async () => {
  ...
  // nothing in common with the team's. The library was fixed for this; `repertory
  // brief` was later wired straight to the filesystem and reintroduced it.
  const bin = await readFile(fileURLToPath(new URL("../../bin/repertory.ts", import.meta.url)), "utf8");
  ...
  assert.ok(!/repertory init/.test(cmd), "brief must never send a bound repo to init");
});
...
test("a new memory is created on main, not on whatever git defaults to", async () => {
  ...
  // not look like a repertory memory. Silent, and only ever hit by the second person.
  await withHome(async (home: string) => {
...
test("linking writes a SessionStart hook without touching the project's own settings", async () => {
  ...
  await withHome(async (home: string) => {
    ...
    assert.equal(commands.filter((c: string) => c.includes("repertory brief")).length, 1);
  });
});
...
test("a missing LOCAL memory is not reported as a permissions problem", async () => {
  ...
  await withHome(async (home: string) => {
    ...
    assert.ok(!/repertory invite/i.test(reason), reason);
